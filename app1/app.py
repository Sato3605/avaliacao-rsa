...
webhooks = {
    'app2': 'http://127.0.0.1:5001/webhook_message'  # URL do webhook do app2
}
...
@app.route('/')
def index():
    return render_template('chat.html', public_key=publica_srv, app_name="App1")

@socketio.on('connect')
def handle_connect():
    sid = request.sid
    print(f"[App1] Cliente conectado: {sid}")
...
def handle_disconnect():
    ...
    if sid in clients:
        del clients[sid]
    print(f"[App1] Cliente desconectado: {sid}")
...
def receive_client_key(data):
    ...
    print(f"[App1] Recebeu chave pública de {sid}")
...
def handle_send_message(data):
    ...
    # Envia para app2 via webhook
    try:
        webhook_url = webhooks['app2']
...
def webhook_message():
    ...
    print(f"[App1] Mensagem recebida via webhook: {mensagem}")

    # Envia para todos os clientes do app1
    for sid, info in clients.items():
        pubkey = info['public_key']
